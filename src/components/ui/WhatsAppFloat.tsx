  ...
  return (
    <a
      href={`https://wa.me/${COMPANY.whatsapp}?text=${encodeURIComponent(
        "Halo Hikmah Catering, saya ingin bertanya mengenai layanan katering."
      )}`}
      target="_blank"
    ...
    >
      ...
      <div className="absolute bottom-full right-0 mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
        <div className="bg-slate-900 text-white text-xs px-3 py-2 rounded-lg whitespace-nowrap shadow-lg">
          Chat via WhatsApp
          <div className="absolute bottom-0 right-5 translate-y-1/2 rotate-45 w-2 h-2 bg-slate-900" />
        </div>
      </div>
    </a>
  );
